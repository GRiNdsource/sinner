...
Server_sinner = io.popen("echo $SSH_CLIENT | awk '{ print $1}'"):read('*a')
------------------------------------------------------------------------------------------------------------
local function Load_File()
local f = io.open("./Info_Sudo.lua", "r")  
if not f then   
if not redis:get(Server_sinner.."Token_Devsinner") then
...
redis:set(Server_sinner.."Token_Devsinner",token)
end 
else
io.write('\n\27[1;31m⌯︙Token was not saved \n لم يتم حفظ التوكن \n\27[0;39;49m')
end 
os.execute('lua sinner.lua')
end
------------------------------------------------------------------------------------------------------------
if not redis:get(Server_sinner.."User_Devsinner1") then
io.write('\n\27[1;35m⌯︙Send ID For Sudo : ارسل ايدي المطور الاساسي ...\n\27[0;39;49m')
local User_Sudo = io.read():gsub('@','')
if User_Sudo ~= '' then
io.write('\n\27[1;31m⌯︙The ID Is Saved : تم حفظ ايدي المطور\n\27[0;39;49m')
redis:set(Server_sinner.."Id_Devsinner",User_Sudo)
io.write('\n\27[1;35m⌯︙Send UserName For Sudo : ارسل معرف المطور الاساسي ...\n\27[0;39;49m')
local User_Sudo2 = io.read():gsub('@','')
if User_Sudo ~= '' then
redis:set(Server_sinner.."User_Devsinner1",User_Sudo2)
end
else
...
------------------------------------------------------------------------------------------------------------
local Devsinner_Info_Sudo = io.open("Info_Sudo.lua", 'w')
Devsinner_Info_Sudo:write([[
do 
local sinner_INFO = {
Id_Devsinner = ]]..redis:get(Server_sinner.."Id_Devsinner")..[[,
UserName_sinner = "]]..redis:get(Server_sinner.."User_Devsinner1")..[[",
Token_Bot = "]]..redis:get(Server_sinner.."Token_Devsinner")..[["
}
return sinner_INFO
end

]])
Devsinner_Info_Sudo:close()
------------------------------------------------------------------------------------------------------------
local Run_File_sinner = io.open("sinner", 'w')
Run_File_sinner:write([[
#!/usr/bin/env bash
cd $HOME/sinner
token="]]..redis:get(Server_sinner.."Token_Devsinner")..[["
while(true) do
rm -fr ../.telegram-cli
./tg -s ./sinner.lua -p PROFILE --bot=$token
done
]])
Run_File_sinner:close()
------------------------------------------------------------------------------------------------------------
...
while(true) do
rm -fr ../.telegram-cli
screen -S sinner -X kill
screen -S sinner ./sinner
done
]])
Run_SM:close()
io.popen("mkdir Files")
os.execute('chmod +x tg')
os.execute('chmod +x sinner')
os.execute('chmod +x tk')
os.execute('./tk')
Status = true
else   
f:close()  
redis:del(Server_sinner.."Token_Devsinner");redis:del(Server_sinner.."Id_Devsinner");redis:del(Server_sinner.."User_Devsinner1")
Status = false
end  
return Status
end
...
UserName_Dev = sudos.UserName_sinner
bot_id = token:match("(%d+)")  
Id_Dev = sudos.Id_Devsinner
Ids_Dev = {sudos.Id_Devsinner,bot_id}
Name_Bot = redis:get(bot_id.."Redis:Name:Bot") or "سينر"
------------------------------------------------------------------------------------------------------------
function var(value)  
...
------------------------------------------------------------------------------------------------------------
function Dev_sinner(msg)  
local Dev_sinner = false  
...
function Dev_sinner_User(user)  
local Dev_sinner_User = false  
for k,v in pairs(Ids_Dev) do  
if user == v then  
Dev_sinner_User = true  
end  
end  
return Dev_sinner_User  
end 
function DeveloperBot(msg)  
local Status = redis:sismember(bot_id.."Developer:Bot", msg.sender_user_id_) 
if Status or Dev_sinner(msg) or Bot(msg) then  
return true  
else  
return false  
end  
...
if hash or Dev_sinner(msg) or DeveloperBot(msg) or Bot(msg) then  
return true 
else 
return false 
end 
...
if hash or Dev_sinner(msg) or DeveloperBot(msg) or PresidentGroup(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_sinner(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_sinner(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Constructor(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_sinner(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Constructor(msg) or Owner(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_sinner(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Constructor(msg) or Owner(msg) or Admin(msg) or Bot(msg) then     
return true 
else 
return false 
end 
...
local url , res = https.request('https://mpqm.ml/source/api/ch.php?ch=@WWTUP&id='..User..'')
data = JSON.decode(url)
if data.Ch_Member.WwTuP ~= true then
Var = false
else
Var = true
end
...
if Dev_sinner_User(user_id) then
Status = true  
...
if Dev_sinner_User(user_id) == true then
...
url = 'https://raw.githubusercontent.com/GRiNdsource/sinner/master/sinner.lua'
file_path = 'sinner.lua'
local respbody = {} 
...
url = 'https://raw.githubusercontent.com/GRiNdsource/sinner/master/sinner.lu'
file_path = 'sinner.lua'
local respbody = {} 
local options = { url = url, sink = ltn12.sink.table(respbody), redirect = true } 
local response = nil 
...
local UserName = (data.username_ or "WwTuP")
for gmatch in string.gmatch(data.first_name_, "[^%s]+") do
data.first_name_ = gmatch or 'sinner'
end
...
------------------------------------------------------------------------------------------------------------
function Filessinner(msg)
File_Bot = dofile("sinner.lua")
if File_Bot.sinner and msg then
Text_File = File_Bot.sinner(msg)
end
send(msg.chat_id_, msg.id_,Text_File)  
return false
end
function FilessinnerBot(msg)
for v in io.popen('ls Files'):lines() do
if v:match(".lua$") then
Text_FileBot = dofile("Files/"..v)
if Text_FileBot.sinnerFile and msg then
Text_FileBot = Text_FileBot.sinnerFile(msg)
end
end
end
...
------------------------------------------------------------------------------------------------------------
function Dev_sinner_File(msg,data)
if msg then
msg = data.message_
...
send(msg.chat_id_, msg.id_,'هلا بتاجراسي ومطوري مسلم.')
return false 
end
...
------------------------------------------------------------------------------------------------------------
if text and redis:get(bot_id..'GetTexting:Devsinner'..msg.chat_id_..':'..msg.sender_user_id_) then
if text == 'الغاء' or text == 'الغاء ✖' then 
redis:del(bot_id..'GetTexting:Devsinner'..msg.chat_id_..':'..msg.sender_user_id_)
send(msg.chat_id_,msg.id_,'⌯︙تم الغاء حفظ كليشة المطور')
return false
end
redis:set(bot_id..'Texting:Devsinner',text)
redis:del(bot_id..'GetTexting:Devsinner'..msg.chat_id_..':'..msg.sender_user_id_)
...
if Dev_sinner(msg) then
if text == 'نقل الاحصائيات' then
local Users = redis:smembers(bot_id.."sinner:UsersBot")
local Groups = redis:smembers(bot_id..'sinner:Chek:Groups') 
for i = 1, #Groups do
redis:sadd(bot_id..'ChekBotAdd',Groups[i])  
local list1 = redis:smembers(bot_id..'sinner:Basic:Constructor'..Groups[i])
for k,v in pairs(list1) do
redis:sadd(bot_id.."President:User"..Groups[i], v)
end
local list2 = redis:smembers(bot_id..'sinner:Constructor'..Groups[i])
for k,v in pairs(list2) do
redis:sadd(bot_id.."Constructor:Group"..Groups[i], v)
end
local list3 = redis:smembers(bot_id..'sinner:Manager'..Groups[i])
for k,v in pairs(list3) do
redis:sadd(bot_id.."Manager:Group"..Groups[i], v)
end
local list4 = redis:smembers(bot_id..'sinner:Mod:User'..Groups[i])
...
download_to_file('https://raw.githubusercontent.com/sinnerTeam/sinner/master/sinner.lua','sinner.lua') 
...
local Get_Files, res = https.request("https://raw.githubusercontent.com/sinnerTeam/Files_sinner/master/getfile.json")
if res == 200 then
local Get_info, res = pcall(JSON.decode,Get_Files);
if Get_info then
local TextS = "\n⌯︙قائمه ملفات متجر سورس sinner\n⌯︙الملفات المتوفره حاليا\n━━━━━━━━━━━━━\n\n"
...
local Get_Json, Res = https.request("https://raw.githubusercontent.com/sinnerTeam/Files_sinner/master/Files_sinner/"..File_Name)
if Res == 200 then
...
if not Dev_sinner(msg) and not redis:sismember(bot_id..'User:Ban:Pv',msg.sender_user_id_) and not redis:get(bot_id..'Status:Lock:Twasl') then
...
redis:set(bot_id..'GetTexting:Devsinner'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
redis:del(bot_id..'Texting:Devsinner')
...
if Dev_sinner_User(result.id_) == true then
...
send(msg.chat_id_, msg.id_,'*⌯︙ عدد المجموعات الان ← { '..#group..' } مجموعه '..sinner..''..taha..'\n⌯︙اصبح عدد المجموعات الان ← { '..sendok..' } مجموعات*\n')   
end
end
end,nil)
end
...
if text == 'رفع النسخه الاحتياطيه' and tonumber(msg.reply_to_message_id_) > 0 and Dev_sinner(msg) then   
tdcli_function({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},function(Arg, Data)   
...
if text == ("اضف مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_sinner(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌯︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌯︙قناة البوت 📁.\n- @WwTuP')   
return false
end
...
if text == ("حذف مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_sinner(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌯︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌯︙قناة البوت 📁.\n- @WwTuP')   
return false
end
...
if text and text:match("^اضف مطور @(.*)$") and Dev_sinner(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌯︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌯︙قناة البوت 📁.\n- @WwTuP')   
return false
end
...
if text and text:match("^حذف مطور @(.*)$") and Dev_sinner(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌯︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌯︙قناة البوت 📁.\n- @WwTuP')   
return false
end
...
if text and text:match("^اضف مطور (%d+)$") and Dev_sinner(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌯︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌯︙قناة البوت 📁.\n- @WwTuP')   
return false
end
...
if text and text:match("^حذف مطور (%d+)$") and Dev_sinner(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌯︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌯︙قناة البوت 📁.\n- @WwTuP')   
return false
end
...
if text == 'جلب نسخه احتياطيه' and Dev_sinner(msg) or text == 'جلب نسخه الكروبات' and Dev_sinner(msg) then
local list = redis:smembers(bot_id..'ChekBotAdd')  
...
if text == ("مسح قائمه العام") and Dev_sinner(msg) or text == ("مسح المحظورين عام") and Dev_sinner(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌯︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌯︙قناة البوت 📁.\n- @WwTuP')   
return false
end
redis:del(bot_id.."Removal:User:Groups")
send(msg.chat_id_, msg.id_, "⌯︙تم مسح المحظورين عام من البوت")
elseif text == ("مسح المطورين") and Dev_sinner(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌯︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌯︙قناة البوت 📁.\n- @WwTuP')   
return false
end
...
elseif text == ("قائمه العام") and Dev_sinner(msg) or text == ("المحظورين عام") and Dev_sinner(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌯︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌯︙قناة البوت 📁.\n- @WwTuP')   
return false
end
...
send(msg.chat_id_, msg.id_, Gban)
elseif text == ("المطورين") and Dev_sinner(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌯︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌯︙قناة البوت 📁.\n- @WwTuP')   
return false
end
...
elseif text == ("حظر عام") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_sinner(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌯︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌯︙قناة البوت 📁.\n- @WwTuP')   
return false
end
...
if Dev_sinner_User(result.sender_user_id_) == true then
send(msg.chat_id_, msg.id_, "⌯︙لا تستطيع حظر المطور الاساسي عام")
return false 
end
...
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
elseif text == ("الغاء العام") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_sinner(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌯︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌯︙قناة البوت 📁.\n- @WwTuP')   
return false
end
...
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
elseif text and text:match("^حظر عام @(.*)$") and Dev_sinner(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌯︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌯︙قناة البوت 📁.\n- @WwTuP')   
return false
end
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^حظر عام @(.*)$")}, FunctionStatus, nil)
elseif text and text:match("^الغاء العام @(.*)$") and Dev_sinner(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌯︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌯︙قناة البوت 📁.\n- @WwTuP')   
return false
end
...
elseif text and text:match("^حظر عام (%d+)$") and Dev_sinner(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌯︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌯︙قناة البوت 📁.\n- @WwTuP')   
return false
end
if Dev_sinner_User(text:match("^حظر عام (%d+)$")) == true then
send(msg.chat_id_, msg.id_, "⌯︙لا تستطيع حظر المطور الاساسي عام")
return false 
end
...
elseif text and text:match("^الغاء العام (%d+)$") and Dev_sinner(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌯︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌯︙قناة البوت 📁.\n- @WwTuP')   
return false
end
...
elseif text == "تفعيل المغادره" and Dev_sinner(msg) then   
redis:del(bot_id.."Status:Lock:Left"..msg.chat_id_)  
send(msg.chat_id_, msg.id_,"⌯︙تم تفعيل مغادرة البوت") 
elseif text == "تعطيل المغادره" and Dev_sinner(msg) then  
redis:set(bot_id.."Status:Lock:Left"..msg.chat_id_,true)   
send(msg.chat_id_, msg.id_, "⌯︙تم تعطيل مغادرة البوت") 
elseif text == "تفعيل الاذاعه" and Dev_sinner(msg) then  
redis:del(bot_id.."Status:Broadcasting:Bot") 
send(msg.chat_id_, msg.id_,"⌯︙تم تفعيل الاذاعه \n⌯︙الان يمكن للمطورين الاذاعه" ) 
elseif text == "تعطيل الاذاعه" and Dev_sinner(msg) then  
redis:set(bot_id.."Status:Broadcasting:Bot",true) 
send(msg.chat_id_, msg.id_,"⌯︙تم تعطيل الاذاعه") 
...
elseif text == 'تفعيل البوت الخدمي' and Dev_sinner(msg) then  
redis:del(bot_id..'Free:Bot') 
send(msg.chat_id_, msg.id_,'⌯︙تم تفعيل البوت الخدمي \n⌯︙الان يمكن الجميع تفعيله') 
elseif text == 'تعطيل البوت الخدمي' and Dev_sinner(msg) then  
redis:set(bot_id..'Free:Bot',true) 
send(msg.chat_id_, msg.id_,'⌯︙تم تعطيل البوت الخدمي') 
...
elseif text == 'وضع كليشه المطور' and Dev_sinner(msg) then
redis:set(bot_id..'GetTexting:Devsinner'..msg.chat_id_..':'..msg.sender_user_id_,true)
send(msg.chat_id_,msg.id_,'⌯︙ ارسل لي الكليشه الان')
...
elseif text == 'روابط الكروبات' and Dev_sinner(msg) then
local list = redis:smembers(bot_id..'ChekBotAdd') 
test = '⌯︙روابط الكروبات \n\n'
for k,v in pairs(list) do 
...
elseif text == ("مسح ردود المطور") and Dev_sinner(msg) then 
...
elseif text == ("ردود المطور") and Dev_sinner(msg) then 
local list = redis:smembers(bot_id.."List:Rd:Sudo")
text = "\n⌯︙قائمة ردود المطور \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(list) do
...
send(msg.chat_id_, msg.id_,"["..text.."]")
elseif text == "اضف رد للكل" and Dev_sinner(msg) then 
...
elseif text == "حذف رد للكل" and Dev_sinner(msg) then 
...
if Dev_sinner_User(msg.sender_user_id_) == true then
redis:srem(bot_id.."Developer:Bot", result.id_)
...
elseif text == "تغير اسم البوت" and Dev_sinner(msg) or text == "تغيير اسم البوت" and Dev_sinner(msg) then 
redis:setex(bot_id.."Change:Name:Bot"..msg.sender_user_id_,300,true) 
send(msg.chat_id_, msg.id_,"⌯︙ ارسل لي الاسم الان ")  
elseif text=="اذاعه خاص" and msg.reply_to_message_id_ == 0 and DeveloperBot(msg) then 
if redis:get(bot_id.."Status:Broadcasting:Bot") and not Dev_sinner(msg) then 
...
tdcli_function ({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},reply, nil)
return false
elseif text == "تنظيف المشتركين" and Dev_sinner(msg) then
local pv = redis:smembers(bot_id..'Num:User:Pv')  
local sendok = 0
for i = 1, #pv do
tdcli_function({ID='GetChat',chat_id_ = pv[i]},function(arg,dataq)
...
return false
elseif text == "تنظيف الكروبات" and Dev_sinner(msg) then
local group = redis:smembers(bot_id..'ChekBotAdd')  
local w = 0
local q = 0
for i = 1, #group do
...
local UserName = (b.username_ or "WwTuP")
...
elseif text and text:match("^تعين عدد الاعضاء (%d+)$") and Dev_sinner(msg) then
redis:set(bot_id..'Num:Add:Bot',text:match("تعين عدد الاعضاء (%d+)$") ) 
send(msg.chat_id_, msg.id_,'*⌯︙ تم تعيين عدد اعضاء تفعيل البوت اكثر من : '..text:match("تعين عدد الاعضاء (%d+)$")..' عضو *')
elseif text =='الاحصائيات' and DeveloperBot(msg) then 
send(msg.chat_id_, msg.id_,'*⌯︙عدد احصائيات البوت الكامله \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n⌯︙عدد المجموعات : '..(redis:scard(bot_id..'ChekBotAdd') or 0)..'\n⌯︙عدد المشتركين : '..(redis:scard(bot_id..'Num:User:Pv') or 0)..'*')
elseif text == 'المطور' or text == 'مطور' then
local TextingDevsinner = redis:get(bot_id..'Texting:Devsinner')
if TextingDevsinner then 
send(msg.chat_id_, msg.id_,TextingDevsinner)
else
send(msg.chat_id_, msg.id_,'['..UserName_Dev..']')
end
elseif text == 'حذف كليشه المطور' and Dev_sinner(msg) then
redis:del(bot_id..'Texting:Devsinner')
send(msg.chat_id_, msg.id_,'⌯︙ تم حذف كليشه المطور')
end
end
...
tdcli_function ({ ID = "GetChannelFull", channel_id_ = msg.chat_id_:gsub("-100","")}, function(arg,data)  
if tonumber(data.member_count_) < tonumber(redis:get(bot_id..'Num:Add:Bot') or 0) and not Dev_sinner(msg) then
...
--------------------------------------------------------------------------------------------------------------
Dev_sinner_File(msg,data)
FilessinnerBot(msg,data)
